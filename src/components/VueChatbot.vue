<template>
  ...
  >
    <!-- soft gloss overlay -->
    <div
      class="pointer-events-none absolute inset-0 rounded-3xl bg-gradient-to-br from-white/30 via-white/10 to-transparent mix-blend-screen opacity-40"
    ></div>
    ...
    <div ref="chatContainer" class="h-64 overflow-y-auto pr-2 space-y-2">
      ...
      >
        <div
          ...
          :class="[
            'relative px-4 py-2 rounded-3xl max-w-xs text-sm font-sans whitespace-pre-line',
            msg.role === 'user' ? 'bg-blue-500/40 text-white' : 'bg-gray-200/40 text-gray-800',
          ]"
        ></div>
      </div>
    </div>
...
const renderMarkdown = (text: string) => marked.parseInline(text)
